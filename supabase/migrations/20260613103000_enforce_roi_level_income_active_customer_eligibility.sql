-- Tighten ROI-to-ROI eligibility.
-- ROI level income can only be credited to valid active customers, and direct
-- counts must only include valid active direct customers.
...
AS $roi_direct_count$
  ...
  WHERE public.is_valid_roi_active_customer(child.tu_id)
    AND (
...
AS $roi_level_cap$
  WITH launch_plan AS (
    ...
    WHERE us.tus_user_id = p_user_id
      AND public.is_valid_roi_active_customer(p_user_id)
      AND us.tus_status IN ('active', 'upgraded')
      AND (us.tus_end_date IS NULL OR us.tus_end_date > now())
      AND COALESCE(us.tus_plan_phase, sp.tsp_plan_phase, 'prelaunch') = 'launch'
  )
...
BEGIN
  ...
  FOR v_level, v_recipient_id, v_percent, v_required_directs IN
    ...
    WHERE public.is_valid_roi_active_customer(u.recipient_user_id)
