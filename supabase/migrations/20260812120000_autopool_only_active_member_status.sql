-- Treat customers with an active 20 USDT AutoPool subscription as active members.

CREATE OR REPLACE FUNCTION public.has_active_autopool_20_subscription(p_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
SELECT EXISTS (
  ...
  WHERE u.tu_id = p_user_id
    ...
    AND (
      ...
      OR (
        COALESCE(u.tu_registration_paid, false) = true
...
BEGIN
  ...
  WHERE u.tu_user_type = 'customer'
    ...
    AND (
      ...
      OR (v_plan_filter = 'autopool' AND public.has_active_autopool_20_subscription(u.tu_id))
    );
...
REVOKE EXECUTE ON FUNCTION public.has_active_autopool_20_subscription(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.has_active_autopool_20_subscription(uuid) TO authenticated, service_role;
