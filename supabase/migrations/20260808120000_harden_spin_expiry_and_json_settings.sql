-- Keep scheduled spin expiry working without broadening end-user access.
-- Also accept system settings stored as either JSON numbers or JSON strings.
...
BEGIN
  ...
  v_claimed_role := COALESCE(v_claims->>'role', '');

  IF p_user_id IS NULL THEN
    IF v_claimed_role <> 'service_role'
       AND session_user NOT IN ('postgres', 'supabase_admin') THEN
      RAISE EXCEPTION 'Not authorized to expire spin wheel rewards';
    END IF;
  ELSIF v_claimed_role <> 'service_role'
...
BEGIN
  ...
  SELECT COALESCE(
    CASE
      WHEN jsonb_typeof(tss_setting_value::jsonb) IN ('number', 'string')
        THEN NULLIF(tss_setting_value::jsonb #>> '{}', '')::numeric
      ELSE NULL
    END,
    10
  )
