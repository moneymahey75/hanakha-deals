/*
  # Add Admin Get Customers RPC

  1. New Functions
    - `admin_get_customers` - Fetches all customer data for admin dashboard
      - Takes filter parameters (search, status, verification, pagination)
      - Uses SECURITY DEFINER to bypass RLS
      - Returns customer data with profiles
      - Only accessible by authenticated admins

  2. Security
    - Uses SECURITY DEFINER to read customer data without RLS
    - Validates admin authentication before proceeding
    - Returns comprehensive customer information
*/

-- Create function to get customers for admin
