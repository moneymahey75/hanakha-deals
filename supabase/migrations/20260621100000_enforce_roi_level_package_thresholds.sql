-- Enforce ROI-to-ROI package thresholds by recipient level.
-- Levels 1-7 require an active earning 50+ USDT launch package.
-- Levels 8-10 require an active earning 100+ USDT launch package.
-- Levels 11-15 require an active earning 200+ USDT launch package.

CREATE OR REPLACE FUNCTION public.get_required_roi_package_amount_for_level(p_level integer)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
SECURITY DEFINER
...
AS $$
  SELECT CASE
    WHEN p_level BETWEEN 1 AND 7 THEN 50::numeric
    WHEN p_level BETWEEN 8 AND 10 THEN 100::numeric
    WHEN p_level BETWEEN 11 AND 15 THEN 200::numeric
    ELSE NULL::numeric
  END;
$$;
...
AS $$
  SELECT CASE
    WHEN EXISTS (
      SELECT 1
      FROM public.get_user_active_earning_subscriptions(p_user_id) active
      WHERE active.plan_amount >= 200
    ) THEN 15
    WHEN EXISTS (
      SELECT 1
      FROM public.get_user_active_earning_subscriptions(p_user_id) active
      WHERE active.plan_amount >= 100
    ) THEN 10
    WHEN EXISTS (
      SELECT 1
      FROM public.get_user_active_earning_subscriptions(p_user_id) active
      WHERE active.plan_amount >= 50
    ) THEN 7
    ELSE 0
  END;
$$;

REVOKE EXECUTE ON FUNCTION public.get_required_roi_package_amount_for_level(integer) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.get_required_roi_package_amount_for_level(integer) TO authenticated, service_role;
...
SECURITY DEFINER
SET search_path = public
AS $award_roi_level_package_thresholds$
DECLARE
  v_assignment record;
  v_coupon record;
  v_source_label text;
...
BEGIN
  ...
  LOOP
    ...
    FROM public.get_user_active_earning_subscription_for_roi_level(v_recipient_id, v_level) earning
    LIMIT 1;
    ...
    IF COALESCE(v_recipient_valid, false) = false OR v_recipient_subscription_id IS NULL THEN
      INSERT INTO public.tbl_roi_level_incomes (
        ...
        v_max_level,
        'locked',
        'no_qualifying_roi_level_package',
...
END;
$award_roi_level_package_thresholds$;
