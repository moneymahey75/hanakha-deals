...
AS $$
  SELECT EXISTS (
    ...
    LEFT JOIN public.tbl_user_subscriptions us ON us.tus_id = p.tp_subscription_id
    LEFT JOIN public.tbl_subscription_plans sp ON sp.tsp_id = us.tus_plan_id
    WHERE p.tp_user_id = p_user_id
      AND p.tp_payment_status = 'completed'
      AND (
        p.tp_payment_method IN ('reserved_wallet', 'blockchain_plus_reserved')
        OR COALESCE(p.tp_gateway_response, '{}'::jsonb) ? 'chain_paid'
        OR COALESCE(p.tp_gateway_response, '{}'::jsonb) ? 'reserved_used'
      )
      AND (
        sp.tsp_id IS NULL
        OR lower(COALESCE(sp.tsp_type::text, 'upgrade')) = 'upgrade'
      )
  )
  OR (
    EXISTS (
      SELECT 1
      FROM public.tbl_user_subscriptions launch_sub
      JOIN public.tbl_subscription_plans launch_plan ON launch_plan.tsp_id = launch_sub.tus_plan_id
      WHERE launch_sub.tus_user_id = p_user_id
        AND launch_sub.tus_status = 'active'
        AND (launch_sub.tus_end_date IS NULL OR launch_sub.tus_end_date > now())
        AND COALESCE(launch_plan.tsp_plan_phase::text, launch_sub.tus_plan_phase::text, 'prelaunch') = 'launch'
        AND lower(COALESCE(launch_plan.tsp_type::text, '')) = 'upgrade'
    )
    AND (
      EXISTS (
        SELECT 1
        FROM public.tbl_user_subscriptions prelaunch_sub
        WHERE prelaunch_sub.tus_user_id = p_user_id
          AND prelaunch_sub.tus_status = 'upgraded'
          AND COALESCE(prelaunch_sub.tus_plan_phase::text, 'prelaunch') = 'prelaunch'
      )
      OR EXISTS (
        SELECT 1
        FROM public.tbl_payments p
        ...
        WHERE p.tp_user_id = p_user_id
          AND p.tp_payment_status = 'completed'
          AND COALESCE(sp.tsp_plan_phase::text, us.tus_plan_phase::text, 'prelaunch') = 'launch'
          AND lower(COALESCE(sp.tsp_type::text, '')) = 'upgrade'
          AND (
            p.tp_payment_method IN ('reserved_wallet', 'blockchain_plus_reserved')
            OR COALESCE(p.tp_gateway_response, '{}'::jsonb) ? 'chain_paid'
            OR COALESCE(p.tp_gateway_response, '{}'::jsonb) ? 'reserved_used'
...
BEGIN
  ...
  IF NOT COALESCE(public.has_completed_launch_upgrade(v_user_id), false) THEN
    RETURN jsonb_build_object(
      'active', false,
      'hasSpun', false,
      'eligible', false,
      'message', 'Spin wheel is available only after completing your Launch upgrade.'
    );
  END IF;
...
BEGIN
  ...
  IF NOT COALESCE(public.has_completed_launch_upgrade(v_user_id), false) THEN
    RAISE EXCEPTION 'Spin wheel is available only after completing your Launch upgrade.';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.has_completed_launch_upgrade(uuid) TO authenticated, service_role;
