-- Fix AutoPool direct-income setting parsing when tss_setting_value is jsonb.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.tbl_autopool_20_direct_income
    WHERE ta20di_payment_id = p_payment_id
  ) THEN
  ...
  IF v_parent_id IS NULL OR v_parent_id = v_payment.tp_user_id
     OR NOT EXISTS (
       ...
       WHERE tu_id = v_parent_id
         AND COALESCE(tu_is_active, false)
         AND COALESCE(tu_registration_paid, false)
     ) THEN
  ...
  SELECT LEAST(
    ...
    GREATEST(
      0,
      COALESCE(NULLIF(tss_setting_value #>> '{}', '')::numeric, 2)
    )
  )
  INTO v_amount
  ...
  INSERT INTO public.tbl_autopool_20_direct_income (
    ta20di_payment_id, ta20di_subscription_id, ta20di_joined_user_id,
    ta20di_parent_user_id, ta20di_amount
  )
