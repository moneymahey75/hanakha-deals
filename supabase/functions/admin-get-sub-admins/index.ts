...
import { adminHasPermission, requireAdminSession } from '../_shared/adminSession.ts';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'admins', 'read')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: admins.read' }), {
        status: 403,
