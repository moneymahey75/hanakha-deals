...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'companies', 'write')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: companies.write' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!companyId || !companyName || !registrationNumber || !gstin || !officialEmail) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        tc_verification_status: verificationStatus
      })
      .eq('tc_id', companyId);
    ...
    await logAdminAction(supabase, admin.tau_id, 'update_company_details', 'companies', {
      company_id: companyId,
      company_name: companyName,
      verification_status: verificationStatus
    });

    return new Response(JSON.stringify({ success: true, data: { companyId } }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
