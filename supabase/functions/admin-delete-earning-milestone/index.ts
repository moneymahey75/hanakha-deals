...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'mlm', 'delete')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: mlm.delete' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await logAdminAction(supabase, admin.tau_id, 'delete_earning_milestone', 'earning_milestones', {
      milestone_id: id
    });
