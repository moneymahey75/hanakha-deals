...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!data?.success) {
      throw new Error(data?.message || 'Failed to create daily task');
    }

    await logAdminAction(supabase, admin.tau_id, 'create_daily_task', 'daily_tasks', {
      task_type: taskType,
      title,
      task_date: taskDate,
