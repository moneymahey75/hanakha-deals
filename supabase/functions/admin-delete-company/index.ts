...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'companies', 'delete')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: companies.delete' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!companyId) {
      return new Response(JSON.stringify({ success: false, error: 'Missing companyId' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await logAdminAction(supabase, admin.tau_id, 'delete_company', 'companies', {
