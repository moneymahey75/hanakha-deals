...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'dailytasks', 'delete')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: dailytasks.delete' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await logAdminAction(supabase, admin.tau_id, 'delete_daily_task', 'daily_tasks', {
      task_id: taskId
    });
