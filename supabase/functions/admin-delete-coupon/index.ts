...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'coupons', 'delete')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: coupons.delete' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!couponId) {
      return new Response(JSON.stringify({ success: false, error: 'Missing couponId' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await logAdminAction(supabase, admin.tau_id, 'delete_coupon', 'coupons', {
      coupon_id: couponId
    });
