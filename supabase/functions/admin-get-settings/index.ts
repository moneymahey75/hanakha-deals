...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'settings', 'read')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: settings.read' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    let query = supabase.from('tbl_system_settings').select('*');

    if (Array.isArray(keys) && keys.length > 0) {
      query = query.in('tss_setting_key', keys);
    }
