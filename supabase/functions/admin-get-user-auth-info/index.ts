...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await logAdminAction(supabase, admin.tau_id, 'get_user_auth_info', 'user_management', {
      user_id: userId
    });

    return new Response(JSON.stringify({ success: true, data: { success: true, ...userData } }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
