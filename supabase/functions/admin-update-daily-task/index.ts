...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!adminHasPermission(admin, 'dailytasks', 'write')) {
      return new Response(JSON.stringify({ success: false, error: 'Permission denied: dailytasks.write' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const { error } = await supabase
      ...
      .update({
        tdt_task_type: taskType,
        tdt_title: title,
        tdt_description: description,
        tdt_content_url: contentUrl,
        tdt_coupon_id: couponId || null,
        tdt_reward_amount: rewardAmount,
        tdt_task_date: taskDate,
        tdt_expires_at: expiresAt,
        tdt_is_active: isActive
      })
      .eq('tdt_id', taskId);
    ...
    await logAdminAction(supabase, admin.tau_id, 'update_daily_task', 'daily_tasks', {
      task_id: taskId,
      task_type: taskType,
      title,
      task_date: taskDate,
    ...
    return new Response(JSON.stringify({ success: true, data: { taskId } }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
