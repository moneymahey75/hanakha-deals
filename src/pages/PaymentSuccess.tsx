...
const PaymentSuccess: React.FC = () => {
  ...
  const txHash = state?.txHash || params.get('tx');

  useEffect(() => {
    sessionStorage.removeItem('payment_success_state');
    sessionStorage.removeItem('payment_selected_plan_state');
    localStorage.removeItem('payment_selected_plan_state');
  }, []);
  ...
  const openExplorer = () => {
    if (!txHash) return;
    const network = payment?.tp_network || state?.network || '';
    const isMainnet = network === 'BSC Mainnet';
  ...
  const amount = payment?.tp_amount ?? state?.amount;
  const currency = payment?.tp_currency ?? 'USDT';
  const network = payment?.tp_network ?? state?.network ?? 'BSC';
  const reservedUsed = Number(state?.reservedUsed || 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 py-12 px-4">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Your plan payment has been confirmed.</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between text-gray-700">
            <span>Amount</span>
            <span className="font-medium">{amount != null ? amount : '-'} {currency}</span>
          </div>
          {reservedUsed > 0 && (
            <div className="flex justify-between text-gray-700">
              <span>Reserved Used</span>
              <span className="font-medium">{reservedUsed.toFixed(2)} USDT</span>
            </div>
          )}
          <div className="flex justify-between text-gray-700">
            <span>Network</span>
