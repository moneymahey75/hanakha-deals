...
const CouponManagement: React.FC = () => {
  ...
  const resetNewCoupon = () => {
    setNewCoupon({
      ...
      valid_from: new Date().toISOString().split('T')[0],
      valid_until: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      usage_limit: 1000, share_reward_amount: 0.5,
            ...
            {filteredCoupons.map((coupon) => (
                <tr key={coupon.tc_id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className="text-sm text-gray-500 font-mono">{coupon.tc_coupon_code}</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500">{coupon.tc_discount_type === 'percentage' ? 'Percentage' : 'Fixed Amount'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                ...
                <form onSubmit={handleCreateCoupon} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex space-x-2">
                        <input type="text" required value={newCoupon.coupon_code} onChange={(e) => setNewCoupon(prev => ({ ...prev, coupon_code: e.target.value.toUpperCase() }))} className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="SUMMER50" />
                  ...
                  <div>
                    ...
                    <textarea value={newCoupon.description} onChange={(e) => setNewCoupon(prev => ({ ...prev, description: e.target.value }))} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="Describe the coupon offer..." />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Discount Type *</label>
                      <select value={newCoupon.discount_type} onChange={(e) => setNewCoupon(prev => ({ ...prev, discount_type: e.target.value as any }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                    ...
                    <div>
                      ...
                      <input type="number" required min="0" step="0.01" value={newCoupon.discount_value} onChange={(e) => setNewCoupon(prev => ({ ...prev, discount_value: parseFloat(e.target.value) || 0 }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select value={newCoupon.company_id} onChange={(e) => setNewCoupon(prev => ({ ...prev, company_id: e.target.value }))} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                  ...
                  <div>
                    ...
                    <input type="number" required min="1" value={newCoupon.usage_limit} onChange={(e) => setNewCoupon(prev => ({ ...prev, usage_limit: parseInt(e.target.value) || 1000 }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="1000" />
                  </div>
                  <div>
      ...
      <div className="bg-white rounded-xl shadow-sm">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            {!editMode && (
                <button onClick={() => setEditMode(true)} className="bg-orange-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-orange-700 transition-colors flex items-center space-x-2">
                  <Edit className="h-4 w-4" /><span>Edit Coupon</span>
                </button>
            )}
          </div>
        </div>
            ...
            <div className="bg-gradient-to-r from-orange-500 to-red-600 text-white p-6 rounded-xl">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-xl font-bold">
                  {editMode ? (
                      <input type="text" value={editedCoupon.tc_title} onChange={(e) => setEditedCoupon({...editedCoupon, tc_title: e.target.value})} className="w-full bg-transparent border-b border-white/50 focus:border-white focus:outline-none" />
                  ) : coupon.tc_title}
                </h4>
                <span className="bg-white text-orange-600 px-3 py-1 rounded-full text-sm font-bold">
                  {(editMode ? editedCoupon : coupon).tc_discount_type === 'percentage'
                      ? `${(editMode ? editedCoupon : coupon).tc_discount_value}% OFF`
                      : `$${(editMode ? editedCoupon : coupon).tc_discount_value} OFF`}
                </span>
              </div>
              {editMode ? (
                  <textarea value={editedCoupon.tc_description || ''} onChange={(e) => setEditedCoupon({...editedCoupon, tc_description: e.target.value})} className="w-full bg-transparent border-b border-white/50 focus:border-white focus:outline-none text-orange-100 mb-4" rows={3} placeholder="Coupon description" />
              ) : (
              ...
              <div className="bg-white/20 backdrop-blur-sm rounded-lg p-3">
                ...
                {editMode ? (
                    <input type="text" value={editedCoupon.tc_coupon_code} onChange={(e) => setEditedCoupon({...editedCoupon, tc_coupon_code: e.target.value.toUpperCase()})} className="w-full bg-transparent border-b border-white/50 focus:border-white focus:outline-none text-xl font-bold font-mono" />
                ) : (
            ...
            <div className="space-y-6">
              <div className="bg-gray-50 p-6 rounded-lg">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    {editMode ? (
                        <select value={editedCoupon.tc_status} onChange={(e) => setEditedCoupon({...editedCoupon, tc_status: e.target.value as any})} className="border border-gray-300 rounded px-2 py-1">
                          <option value="pending">Pending</option>
                          <option value="approved">Approved</option>
                          <option value="declined">Declined</option>
                          <option value="cancelled">Cancelled</option>
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${coupon.tc_status === 'approved' ? 'bg-green-100 text-green-800' : coupon.tc_status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                          {coupon.tc_status.charAt(0).toUpperCase() + coupon.tc_status.slice(1)}
                        </span>
                    )}
                  </div>
                  <div className="flex justify-between">
                    ...
                    {editMode ? (
                        <select value={editedCoupon.tc_company_id || ''} onChange={(e) => setEditedCoupon({...editedCoupon, tc_company_id: e.target.value || undefined})} className="border border-gray-300 rounded px-2 py-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    {editMode ? (
                        <input type="number" value={editedCoupon.tc_usage_limit} onChange={(e) => setEditedCoupon({...editedCoupon, tc_usage_limit: parseInt(e.target.value) || 0})} className="border border-gray-300 rounded px-2 py-1 w-20" />
                    ) : <span>{coupon.tc_used_count} / {coupon.tc_usage_limit}</span>}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Reward:</span>
                    {editMode ? (
                        <input type="number" step="0.01" value={editedCoupon.tc_share_reward_amount} onChange={(e) => setEditedCoupon({...editedCoupon, tc_share_reward_amount: parseFloat(e.target.value) || 0})} className="border border-gray-300 rounded px-2 py-1 w-20" />
                    ) : <span className="font-medium">{coupon.tc_share_reward_amount} USDT</span>}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Discount Type:</span>
                    {editMode ? (
                        <select value={editedCoupon.tc_discount_type} onChange={(e) => setEditedCoupon({...editedCoupon, tc_discount_type: e.target.value as 'percentage' | 'fixed_amount'})} className="border border-gray-300 rounded px-2 py-1">
                          <option value="percentage">Percentage</option>
                          <option value="fixed_amount">Fixed Amount</option>
                        </select>
                  ...
                  <div className="flex justify-between">
                    ...
                    {editMode ? (
                        <input type="number" step="0.01" value={editedCoupon.tc_discount_value} onChange={(e) => setEditedCoupon({...editedCoupon, tc_discount_value: parseFloat(e.target.value) || 0})} className="border border-gray-300 rounded px-2 py-1 w-20" />
                  ...
                  {(coupon.tc_launch_date || editMode) && (
                      <div className="flex justify-between">
                        ...
                        {editMode ? (
                            <input type="date" value={formatDateForInput(editedCoupon.tc_launch_date || '')} onChange={(e) => setEditedCoupon({...editedCoupon, tc_launch_date: e.target.value})} className="border border-gray-300 rounded px-2 py-1" />
                        ) : coupon.tc_launch_date ? <span>{new Date(coupon.tc_launch_date).toLocaleDateString()}</span> : null}
                      </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-gray-600">Active:</span>
                    {editMode ? (
                        <input type="checkbox" checked={editedCoupon.tc_is_active} onChange={(e) => setEditedCoupon({...editedCoupon, tc_is_active: e.target.checked})} className="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded" />
                    ) : <span>{coupon.tc_is_active ? 'Yes' : 'No'}</span>}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Launch Now:</span>
                    {editMode ? (
                        <input type="checkbox" checked={editedCoupon.tc_launch_now} onChange={(e) => setEditedCoupon({...editedCoupon, tc_launch_now: e.target.checked})} className="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded" />
                  ...
                  {editMode && (
                      <>
                        <div className="flex justify-between">
                          ...
                          <input type="url" value={editedCoupon.tc_website_url || ''} onChange={(e) => setEditedCoupon({...editedCoupon, tc_website_url: e.target.value})} className="border border-gray-300 rounded px-2 py-1 flex-1 ml-2" placeholder="https://example.com" />
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Image URL:</span>
                          <input type="url" value={editedCoupon.tc_image_url || ''} onChange={(e) => setEditedCoupon({...editedCoupon, tc_image_url: e.target.value})} className="border border-gray-300 rounded px-2 py-1 flex-1 ml-2" placeholder="https://example.com/image.jpg" />
                        </div>
                      </>
                  )}
                </div>
              ...
              {editMode && (
                  <div className="bg-gray-50 p-6 rounded-lg">
                    ...
                    <textarea value={editedCoupon.tc_terms_conditions || ''} onChange={(e) => setEditedCoupon({...editedCoupon, tc_terms_conditions: e.target.value})} rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="Enter terms and conditions..." />
                  </div>
              )}

              {editMode && (
                  <div className="flex justify-end space-x-3 pt-4">
                    <button onClick={handleCancel} className="px-4 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors">Cancel</button>
                    <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors flex items-center space-x-2 disabled:opacity-50">
                      {saving ? (
